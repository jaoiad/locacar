...
create table client (
	...
	cl_username varchar(20) not null,
	cl_passw varchar(50) not null
)engine=innodb;
...
create table gestionaire (
	...
	ges_profil int not null,
	ges_username varchar(20) not null,
	ges_passw varchar(50) not null,
	ges_agence int not null
)engine=innodb;
...
alter table locations add constraint cs10 foreign key (loc_categorie) references categorie(cat_id);
...
alter table equiper add constraint cs15 foreign key (equ_option) references option(opt_id);
